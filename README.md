# A **RAG (Retrieval-Augmented Generation)** chatbot experimental project using the Vercel AI SDK

## Brief Overview

A **RAG (Retrieval-Augmented Generation)** chatbot experimental project using the Vercel AI SDK. The chatbot is behaving as a cheerful cooking assistant and answers only from its knowledge base.
...
- **`app/api/resources/route.ts`** - API endpoint that handles resource creation for batch upload.
...
- **`/batch_upload_recipes.py`** - A python program that reads all .txt and .md files from a source directory and uploads uploads them to the chatbot knowledge base by calling the API /api/resources
